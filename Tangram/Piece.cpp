...
void Piece::update() {
    if (InputHandler::Instance()->getMouseButtonState(0) && playing &&
    ...
    if (!InputHandler::Instance()->getMouseButtonState(0) && dragging) {
        offset = Vec2(0, 0);
        offset2 = Vec2(0, 0);
        dragging = false;
        playing = true;
...
int Piece::getArea() {
    return ((m_dimension.getX() *  m_dimension.getY()) + (m_dimension2.getX() *  m_dimension2.getY()) / 100);
}
